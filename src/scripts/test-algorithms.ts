...
import { getExportStatistics } from '../lib/report-exporter';
...
const REAL_STUDENTS: StudentData[] = [
  {
    id: 'u_abc123def456',
...
function runTextNormalizationTests(): void {
...
function runHierarchicalPathTests(): void {
  ...
  const weekNumber = calculateWeekOfMonth(reportDate);
  ...
  const curso = generateCurso(student.division, student.academicYear);
  ...
  const alumno = generateAlumno(student.studentId, student.name);
  ...
  const hierarchicalPath = generateHierarchicalPath(student, 'física', reportDate);
  ...
  const fileName = generateFileName(reportDate, 'física');
...
function runPerformanceTests(): void {
  ...
  const monitor = createProcessingMonitor('performance-test');
  monitor.start();
  
  // Test text normalization performance
  const startNormalization = Date.now();
  const testTexts = Array.from({ length: 1000 }, (_, i) => 
    `Test Text ${i} with Accénts and Special Ch@rs - Educación Física & Química`
  );
  ...
  const testDates = Array.from({ length: 100 }, (_, i) => 
    new Date(Date.now() + (i * 24 * 60 * 60 * 1000)) // 100 different dates
  );
  const paths = testDates.map(date => 
    generateHierarchicalPath(REAL_STUDENTS[0], 'física', date)
  );
  ...
  const weekNumbers = testDates.map(calculateWeekOfMonth);
...
function runRealDataIntegrationTest(): void {
...
function testWeekCalculationAccuracy(): void {
  ...
  testDates.forEach(({ date, expectedWeek, month }) => {
    ...
    const calculatedWeek = calculateWeekOfMonth(testDate);
    const generatedSemana = generateSemana(testDate);
...
async function runExportStatisticsTest(): Promise<void> {
