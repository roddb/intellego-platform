...
export default function FeedbackViewer({ isOpen, onClose, weekStart, subject }: FeedbackViewerProps) {
  ...
  const formatAiAnalysis = (rawAnalysis: string) => {
    ...
    return sections.map((section, idx) => {
      ...
      // Sección de pregunta (Q1-Q5)
      if (isQuestionSection) {
        const questionNum = isQuestionSection[1];
        const parts = section.split(/\n/);
        const nivelLine = parts[0];
        const justificacionContent = parts.slice(1).join('\n').replace(/^Q\d+_JUSTIFICACIÓN:\s*/, '').trim();

        return (
          <div key={idx} className="mb-4 border-l-4 border-indigo-300 pl-4">
            <div className="text-sm font-semibold text-indigo-700 mb-1">
              Pregunta {questionNum}
            </div>
            <div className="text-xs text-indigo-600 mb-2">
              {nivelLine.replace(/Q\d+_NIVEL:\s*/, 'Nivel: ')}
            </div>
      ...
      if (isFortalezas) {
        const content = section.replace('FORTALEZAS:', '').trim();
        ...
        return (
          <div key={idx} className="mb-4">
            ...
            <div className="space-y-2">
              {items.map((item, i) => (
                <div key={i} className="text-sm text-slate-700 pl-6 border-l-2 border-green-200">
                  {item.replace(/^[-•]\s*/, '')}
                </div>
              ))}
            </div>
      ...
      // Sección de mejoras
      if (isMejoras) {
        const content = section.replace('MEJORAS:', '').trim();
        const items = content.split(/\n/).filter(l => l.trim().length > 0);
        ...
        return (
          <div key={idx} className="mb-4">
            ...
            <div className="space-y-2">
              {items.map((item, i) => (
                <div key={i} className="text-sm text-slate-700 pl-6 border-l-2 border-orange-200">
                  {item.replace(/^[-•]\s*/, '')}
                </div>
              ))}
            </div>
      ...
      if (isComentarios) {
        const content = section.replace('COMENTARIOS_GENERALES:', '').trim();
        const paragraphs = content.split(/\n\n+/).filter(p => p.trim().length > 0);

        return (
          <div key={idx} className="mb-4">
            <div className="text-sm font-semibold text-teal-700 mb-2 flex items-center">
              <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-4l-4 4z" />
              </svg>
              Devolución del Reporte
            </div>
      ...
      if (isAnalisisAI) {
        const content = section.replace('ANÁLISIS_AI:', '').trim();
        const paragraphs = content.split(/\n\n+/).filter(p => p.trim().length > 0);
      ...
      return (
        <div key={idx} className="mb-4">
          <p className="text-sm text-slate-700 whitespace-pre-wrap">{section.trim()}</p>
        </div>
      );
    }).filter(Boolean);
  };
